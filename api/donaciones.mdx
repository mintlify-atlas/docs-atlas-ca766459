...
description: "Create and manage food donation records with line items, track states from PENDIENTE through CONFIRMADA, and confirm donations before generating invoices."
---
...
Returns an array of [DonacionResponseDTO](#donacion-response-schema) objects with HTTP `200 OK`.
...
Returns a single [DonacionResponseDTO](#donacion-response-schema) with HTTP `200 OK`. Throws a runtime exception (`404`) if the donation does not exist.
...
Returns the updated [DonacionResponseDTO](#donacion-response-schema) with `estado` set to `CONFIRMADA` and HTTP `200 OK`.
