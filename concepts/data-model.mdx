...
<CardGroup cols={2}>
  ...
  <Card title="Factura" icon="receipt">
    An invoice linked one-to-one with a Donacion. Created automatically on donation submission (type C) or explicitly via the generate endpoint (type A).
  </Card>
</CardGroup>
...
| `nroRemitoProveedor` | `int` | — | Supplier-side remito reference number |
...
| `externalId` | `UUID` | No | Public-facing identifier; set on `@PrePersist`, immutable |
...
| Value | Notes |
|---|---|
| `A` | Generated by the explicit `/api/facturas/generar/{idDonacion}` endpoint |
| `B` | Manual invoice type |
