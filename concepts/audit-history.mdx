...
<Note>
  The audit history endpoints require specific permissions. Accessing `GET /api/donantes/{id}/historial` requires the `AUDITORIA_DONACION_VER` permission, and `GET /api/instituciones/{externalId}/historial` requires `AUDITORIA_INSTITUCION_VER`. Requests without the appropriate authority will receive a `403 Forbidden` response.
</Note>
